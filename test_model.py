...
interpreter_wakeword = tf.lite.Interpreter(model_path='wakeword_final.tflite')
...
interpreter_command = tf.lite.Interpreter(model_path='mha_rnn_final.tflite')
...
class VoiceAssistanceEngine:
    ...
    def predict(self, audio):
        ...
        feature = get_transform_audio(fname)
        if self.flag == 0:
            ...
            out = output_data
            #out = model.predict(feature,verbose=0)
            ...
            if self.towakeword == 30:
                self.flag = 0
                self.towakeword = 0
                filename = 'reply_audio/ketthuc.wav'
                play_audio(filename)
            return results
...
class DemoAction:
    ...
    def __call__(self, x):
        ...
        print(trascript)
        if trascript == 'lisa oi':
            filename = 'reply_audio/sansang.wav'
            play_audio(filename)
        elif trascript == 'den bat':
            filename = 'reply_audio/dendabat.wav'
            play_audio(filename)
        elif trascript == 'den tat':
            filename = 'reply_audio/dendatat.wav'
            play_audio(filename)
        elif trascript == 'den tang sang':
            filename = 'reply_audio/dendatangsang.wav'
            play_audio(filename)
        elif trascript == 'den giam sang':
            filename = 'reply_audio/dendagiamsang.wav'
            play_audio(filename)              
        elif trascript == 'den chuyen mau':
            filename = 'reply_audio/dendachuyenmau.wav'
            play_audio(filename)               
